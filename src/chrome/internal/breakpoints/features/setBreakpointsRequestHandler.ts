...
export class SetBreakpointsRequestHandler implements ICommandHandlerDeclarer {
    private readonly _clientSourceParser = new ClientSourceParser(this._handlesRegistry, this._sourceResolver);
    ...
    public constructor(
        @inject(TYPES.BreakpointsUpdater) protected readonly _breakpointsLogic: BreakpointsUpdater,
    ...
    public async setBreakpoints(args: DebugProtocol.SetBreakpointsArguments, telemetryPropertyCollector?: ITelemetryPropertyCollector): Promise<ISetBreakpointsResponseBody> {
        if (args.breakpoints) {
            ...
            return { breakpoints: await asyncMap(bpRecipesStatus, bprs => this._bpRecipieStatusToClientConverter.toBPRecipeStatus(bprs)) };
        } else {
