...
import { BPRecipeIsBound, BPRecipeIsUnbound } from './bpRecipeStatusForRuntimeSource';
...
export class BPRecipeHasBoundSubstatuses implements IBPRecipeStatus {
    ...
    constructor(
        public readonly recipe: BPRecipeInSource,
        public readonly boundSubstatuses: BPRecipeIsBound[],
        public readonly unboundSubstatuses: BPRecipeIsUnbound[]) {
        if (this.boundSubstatuses.length === 0) {
            breakWhileDebugging();
            throw new Error(`At least the bound substatus for a single runtime source was expected`);
        }
    }
}
...
export function createBPRecipieStatus(recipe: BPRecipeInSource, boundSubstatuses: BPRecipeIsBound[], unboundSubstatuses: BPRecipeIsUnbound[]): IBPRecipeStatus {
