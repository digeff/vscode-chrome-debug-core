    ...
    public async latestExceptionInfo(): Promise<IExceptionInformation> {
        if (this._lastException) {
            ...
            const response: IExceptionInformation = {
                ...
                breakMode: 'unhandled',
                details: {
                    stackTrace: await ifDefinedDo(this._lastException.description,
                        description => new FormattedExceptionParser(this._scriptsLogic, description).parse(), Promise.resolve([])),
                    message,
