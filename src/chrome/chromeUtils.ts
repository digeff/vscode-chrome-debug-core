...
export async function getLaunchedPort(userDataDir: string): Promise<number> {
    ...
    try {
        ...
        const [ portStr ] = activePortArgs.customerContentData.split('\n'); // chrome uses \n regardless of platform in this file
        const port = parseInt(portStr, 10);
        if (isNaN(port)) return Promise.reject(errors.activePortFileContentsInvalid(activePortFilePath, activePortArgs.customerContentData));
        return port;
    } catch (err) {
